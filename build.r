...
for(ii in unique(b$legislature)) {
  ...
  n %v% "constituency" = as.character(sp[ network.vertex.names(n), "kreis" ])
  ...
  if(plot) {
    ...
    save_plot(n, file = paste0("plots/net_at", ii),
  ...
  if(gexf)
    save_gexf(paste0("net_at", ii), n, meta, mode, colors, extra = "constituency")
